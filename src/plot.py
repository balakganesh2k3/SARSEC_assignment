...
def parse_best_config_curve(log_path, target_hidden=100, target_heads=2, target_blocks=2):
    train_losses = []
    val_ndcgs    = []
    ...
    with open(log_path, "r") as f:
        for line in f:
            ...
            if "Transformer blks:" in line:
                ...
                if (current_hidden == target_hidden and
...
def plot_training_curve(train_losses, val_ndcgs,
                        save_path="../results/sasrec_line.png"):
    ...
    ax2.plot(epochs, val_ndcgs, color=color_ndcg, linewidth=2,
             marker="s", markersize=3, linestyle="--", label="Val NDCG@10")
    ...
    ax2.axvline(x=best_epoch, color="grey", linestyle=":",
                linewidth=1.5, label=f"Best epoch ({best_epoch})")
    ax2.text(best_epoch + 0.5, max(val_ndcgs) * 0.95,
             f"best\nepoch {best_epoch}",
    ...
    fig.suptitle(
        "SASRec training curve — best config (blocks=2, hidden=100, heads=2)",
...
if __name__ == "__main__":
    ...
    train_losses, val_ndcgs = parse_best_config_curve(log_path)
    print(f"parsed {len(train_losses)} epochs for best config")

    if train_losses:
        plot_training_curve(train_losses, val_ndcgs)
    else:
